...
import { apiClient, CreateBusRequest, CreateRouteRequest, LoginRequest, RegisterRequest, RouteSearchParams, UpdateBusRequest, UpdateRouteRequest } from '@/lib/api-client';
...
export const useCreateBusMutation = () => {
  ...
  return useMutation({
    mutationFn: (data: CreateBusRequest) => apiClient.bus.create(data, token as string),
    onSuccess: () => {
...
export const useUpdateBusMutation = (id: number) => {
  const { token } = useAuth();
  const queryClient = useQueryClient();
  
  return useMutation({
    mutationFn: (data: UpdateBusRequest) => apiClient.bus.update(id, data, token as string),
    onSuccess: () => {
...
export const useDeleteBusMutation = () => {
  ...
  return useMutation({
    mutationFn: (id: number) => apiClient.bus.delete(id, token as string),
    onSuccess: (_, id) => {
      // Invalidate specific bus and buses list
      queryClient.invalidateQueries({ queryKey: ['bus', id] });
      queryClient.invalidateQueries({ queryKey: ['buses'] });
...
export const useSearchRoutes = (params: RouteSearchParams) => {
  ...
  return useQuery({
    ...
    enabled: !!token && isAuthenticated && !!(params.origin || params.destination),
    select: (data) => data.data
  });
};
...
export const useCreateRouteMutation = () => {
  ...
  return useMutation({
    mutationFn: (data: CreateRouteRequest) => apiClient.route.create(data, token as string),
    onSuccess: () => {
...
export const useUpdateRouteMutation = (id: number) => {
  const { token } = useAuth();
  const queryClient = useQueryClient();
  
  return useMutation({
    mutationFn: (data: UpdateRouteRequest) => apiClient.route.update(id, data, token as string),
    onSuccess: () => {
...
export const useDeleteRouteMutation = () => {
  ...
  return useMutation({
    mutationFn: (id: number) => apiClient.route.delete(id, token as string),
    onSuccess: (_, id) => {
      // Invalidate specific route and routes list
      queryClient.invalidateQueries({ queryKey: ['route', id] });
      queryClient.invalidateQueries({ queryKey: ['routes'] });
